...
к которому он относится. В противном случае информация будет воспринята как не корректная.

При обработке нескольких менеджеров одного отдела, данные будут рассматриваться как корректные.
Пример:

```
Manager, 123, Zoe Hill, 100000000, HR
Manager, 101, John Doe, 3000.5, HR
```
...
В случае когда флаг `--output` не установлен, вывод будет осуществляться в `console` по умолчанию.
